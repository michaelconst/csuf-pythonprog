...
class Game:
    def __init__(self, master, player1=True, player1_img=X_FILE, player2_img=O_FILE):
        self.master = master
        self.frame = Frame(self.master, padx=10, pady=10, background='black', width=500, height=500)
        self.frame.pack()
        ...
        self.player = PLAYER1 if self.player1_turn else PLAYER2
        self.board = [[0 for _ in range(3)] for _ in range(3)]
        player = "Player" + str(self.player)
        self.lbl_player = Label(self.frame, text=player, font="Arial 20 bold",
                                borderwidth=5, highlightbackground="red", background="black", foreground='white')
        self.lbl_player.grid(columnspan=3, sticky=W)
        # import random
    ...
    def process_move(self, w, row, col):
        if self.board[row][col] == 0:
            ...
            prompt = None
            if winner:
                prompt = "Player{} won!".format(winner)
            if prompt:
                self.lbl_player.config(text=prompt)
    ...
    def get_winner(self, board):
        ...
        if empty == 0:
            self.lbl_player.config(text="Draw!")
